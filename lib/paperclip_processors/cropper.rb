module Paperclip
  ...
  class Cropper < Processor
    # Custom cropping with coordinates.
    ...
    def initialize(file, options = {}, attachment = nil)
      ...
      @x                   = 40 - attachment.instance.xoffset.to_i
      @x                   = @x >= 0 ? "+#{@x}" : "-#{@x.abs}"
      @y                   = 40 - attachment.instance.yoffset.to_i
      @y                   = @y >= 0 ? "+#{@y}" : "-#{@y.abs}"
    ...
    def transformation_command
      new_width = @target_geometry.width.round
      scale = "#{new_width}x#{new_width}"
      crop = ["#{400}x#{400}", @x, @y].join()
      trans = []
